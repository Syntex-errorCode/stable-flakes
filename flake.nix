    ...
    { self, ... }:
    {
      lib = {
        ...
        md = ''
          ...
          decade-long period of stagnation, finally recognized as Stable by
          virtually all vested parties. This means we can finally move on, and
          begin to isolate and remove the hacks and the vast mass of technical
          debt that has been supporting the legacy interface, preventing
          progress, and damaging the UX. This is the end of Drama. We may now
          start addressing the long-standing usability issues, such as the
          support for Dependency Injection, proper Versioning, and easier
          composition of Out-of-Tree Projects. The first step, of course, is to
          delete your `flake.nix` and switch to a proper lockfile management
          solution[^2][^3]. Do not forget the prior art on the
          subject![^4][^5][^6][^7]
